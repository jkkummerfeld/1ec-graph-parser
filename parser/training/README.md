...
- Model names (in the stage files), resultsarc.model.gz, results.trace.model.gz, etc
...
Finally, I tuned the various pruning cutoffs in the stage files (e.g. pruningRatioTrain).
The configuration provided here has verbosityTrain = 512, which means it will print information that can be used to figure out the right pruning cutof.
...
Before building the tagger I had a simple classifier implemented, which is what I actually used to train the model.
...
Note, this is trained with 4 threads in the configuration below.
...
Note, this is trained with 3 threads in the configuration below.
...
## One-endpoint Crossing Spine Parser
